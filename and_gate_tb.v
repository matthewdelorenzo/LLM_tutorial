...
module tb();
    ...
    // Verification: XORs on the right makes any X in good_vector match anything,
    // but X in dut_vector will only match X.
    assign tb_match = ({ out_ref } === ({ out_ref } ^ { out_dut } ^ { out_ref }));
    // Use explicit sensitivity list here. @(*) causes NetProc::nex_input() to be called
    // when trying to compute the sensitivity list of the @(strobe) process,
